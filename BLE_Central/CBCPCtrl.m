...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    ...
    if (!error) {
        ...
        [self.delegate updateCPLog:[NSString stringWithFormat:@"Service discovery on peripheral : %@ UUID: %@ OK !", peripheral, peripheral.UUID]]; 
        int i = 0;
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error {
    ...
    if (!error) {
        ...
        [self.delegate updateCPLog:[NSString stringWithFormat:@"Service discovery on peripheral : %@ UUID: %@ Failed !", peripheral, peripheral.UUID]];
    }
    ...
    for ( CBService *service in peripheral.services ) {
        ...
        if([service.UUID isEqual:[CBUUID UUIDWithString:sUUID]]) {
            for ( CBCharacteristic *characteristic in service.characteristics ) {
                ...
                [self.delegate updateCPLog:[NSString stringWithFormat:@"Characteristic %@",characteristic]];
                if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:cUUID]]) {
